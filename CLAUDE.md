...
### Verified badge redesign: gear + checkmark, icon-only (no migration)

A last-minute, founder-approved design change to `VerifiedBadge.tsx`
(unchanged in behavior since migration `0023` until now): the plain
`✓ Vérifié` text pill is replaced by a purple gear ("engrenage") with a
white checkmark inside, rendered alone — **no adjacent "Vérifié"/
"Verified" text anywhere**. Same trigger (`users.createur_verifie`),
same single shared component, same `tone` prop shape; only the visual
treatment and the text-vs-icon-only question changed.

**Both shapes are single, solid vectors — no stroked outlines, no
layered shapes, same discipline as this app's own crown logo mark
(`Logo.tsx`)**: computed once via a throwaway Node script using real
trigonometry/vector math (never hand-picked pixel coordinates), then the
resulting `d`/`points` strings are pasted into the component as static
constants — nothing is recomputed at render time.

- **Gear**: a single closed `<path>`, 20 teeth. Generated by walking
  around the circle once; each tooth is two straight radial lines (body
  radius → tip radius → tip radius → body radius) connected by a
  straight line across the flat tip, then a circular arc (`A`) along the
  body radius over to the next tooth's base. A tooth's two radial sides
  share the exact same angle at both radii, which is what makes them
  come out as true straight radial lines rather than curved/tapered —
  matching the blocky, straight-sided teeth in the reference image.
  `fill` only, no `stroke`.
- **Checkmark**: a single `<polygon>`, six vertices, built as a
  constant-width mitered stroke of a 3-point centerline (short-arm tip →
  elbow → long-arm tip) — butt caps at the two open ends, a sharp miter
  join at the elbow. Computed via perpendicular-offset + line-line
  intersection vector math, which is what actually *guarantees* both
  arms come out the same width (the brief's own explicit requirement) —
  hand-picking six points by eye could not make that guarantee.
  `fill="#ffffff"`, no `stroke`, so both open ends render as flat,
  square-cut edges (no `strokeLinecap: round`, no capsule look) and the
  elbow renders as a genuine sharp point, never a rounded corner —
  verified directly with a zoomed-in render before trusting it, not
  just assumed from the math.

**`tone` still exists but means something different now that the badge
is a fully opaque two-color icon, not a translucent pill.** The gear
uses `currentColor` (this app's standing convention for its hand-made
icons, see `icons.tsx`/`navIcons.tsx`) via a `text-brand-500` Tailwind
class — so it still follows `--color-brand-500`'s own light/dark-mode
`light-dark()` override for free, same as every brand-purple element in
this app, with zero tone-specific color swap needed. `light` (a plain
card background) needs nothing more. `onDark` (`CreateurProfileView`'s
header — a solid brand-gradient or an arbitrary uploaded cover photo)
adds a small `drop-shadow` only: unlike the old translucent pill (which
could turn genuinely invisible against a similarly-toned background, the
exact bug `DonorBadge` had to work around — see that component's own
`tone` comment), an opaque icon can't actually disappear; a light shadow
is just enough to keep its silhouette crisp against a busy background,
not a structural fix the way `DonorBadge`'s was. `DonorBadge.tsx`'s own
comment ("Same tone-based styling as VerifiedBadge") is now slightly
stale — it still uses the old translucent-pill pattern for its own,
unrelated palier icon+label, which this change didn't touch — flagged
here rather than silently left to confuse a future reader, but not
"fixed," since rewriting `DonorBadge`'s own design wasn't asked for.

**Text label removed — icon-only, per explicit instruction.** The only
real call site left (`CreateurProfileView.tsx`'s header, next to the
`<h1>` display name) used to pass `label={t("verified")}` to render as
visible pill text; that `label` prop is now used exclusively as the
SVG's `aria-label` (`<svg role="img" aria-label={label}>`) — a screen
reader still announces "Vérifié"/"Verified", a sighted visitor sees only
the icon. No `<span class="sr-only">` needed: `role="img"` +
`aria-label` on the SVG itself is the standard, minimal-DOM accessible
pattern for an icon that *is* the whole content, not decoration next to
visible text (unlike this app's other hand-made icons in
`icons.tsx`/`navIcons.tsx`, which stay `aria-hidden` because they always
sit next to their own visible label). The call site itself
(`CreateurProfileView.tsx`) needed **no changes at all** — it already
passed `label`/`tone` positionally, which now just get used
differently inside the component.

**`/explorer` was checked and confirmed to have no `VerifiedBadge` call
site to update** — despite this component's own original doc comment
still saying "shared between `CreateurProfileView` and `/explorer`'s
cards," that stopped being true once Explorer became a publications grid
(see "Explorer becomes a publications grid" above): the créateur-card
list `/explorer` used to render (with its own badge) doesn't exist
there anymore, replaced entirely by publication tiles with no
créateur-verification badge of their own. Confirmed by grepping the
whole `src/` tree for `VerifiedBadge` before touching anything — exactly
one real call site remains, `CreateurProfileView.tsx`. The component's
own top comment was corrected to say so, rather than left describing a
call site that no longer exists.

**No test depended on the removed text** — grepped every `*.test.ts(x)`
file for `VerifiedBadge`/`"verified"`/`"Vérifié"` before starting; none
exist (this project has no jsdom/testing-library, so a component like
this was never rendered in a vitest test to begin with — see this file's
own standing "pure helper in lib, DOM in the component, no way to test
the DOM part in vitest" pattern). Nothing needed updating there.

Verified visually end-to-end, twice, for two different things: (1) the
gear/checkmark geometry itself, via a standalone HTML preview (no app
involved) rendered at 400px/96px/24px to confirm the sharp elbow, flat
square-cut arm ends, consistent arm width, and legibility at the real
~24px display size, plus both `tone` values against a plain light
background, a dark-mode card background, and a brand-gradient banner
(only `onDark` is reachable live today, since `/explorer` no longer has
a `light`-tone call site — the `light` tone was checked this way instead
of skipped); (2) the real, running app (`next dev` + a small mock
`profils_publics` REST endpoint returning a fixture créateur with
`createur_verifie: true`, everything else empty — the header is the only
part of the page this check needed), confirming the badge renders at a
real 24×24px on `/createur/[id]`, with the correct localized
`aria-label` (`"Vérifié"`/`"Verified"`) and the `<h1>`'s own text content
containing only the display name, nothing appended — in all 4 of
`fr`/`en` × light/dark, plus a zoomed crop of the live-rendered badge
