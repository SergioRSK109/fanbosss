...
function buildRequest(body: unknown) {
  return new Request("http://localhost/api/admin/suspendre-compte", {
    method: "POST",
...
describe("POST /api/admin/suspendre-compte", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("rejects an unauthenticated request", async () => {
    ...
    const { POST } = await import("@/app/api/admin/suspendre-compte/route");
  ...
  it("surfaces the RPC's rejection (e.g. caller isn't admin, or already suspended) as a 403", async () => {
    vi.mocked(createSupabaseServerClient).mockResolvedValue(
  ...
  it("passes userId/raison through to suspendre_compte, trimming a blank raison to null", async () => {
    const rpcSpy = vi.fn();
    vi.mocked(createSupabaseServerClient).mockResolvedValue(
    ...
    const response = await POST(
      buildRequest({ userId: "u2", raison: "  contenu inapproprié répété  " }) as never,
    );

    expect(response.status).toBe(200);
    expect(rpcSpy).toHaveBeenCalledWith("suspendre_compte", {
      p_user_id: "u2",
      p_raison: "contenu inapproprié répété",
    });
  });
