...
export async function PATCH(request: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", user.id)
    .select(
      "id, nom_affichage, pseudo, pseudo_modifie_at, bio, lien_reseau_social, lien_tiktok, lien_instagram, lien_youtube, lien_autre, classement_public, masque_exploration, badge_fidelite_public",
    )
    .single();
