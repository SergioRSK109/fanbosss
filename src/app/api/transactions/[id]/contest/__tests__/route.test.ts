...
function buildRequest(id: string) {
  return {
    request: new Request(`http://localhost/api/transactions/${id}/contest`, { method: "POST" }),
    params: Promise.resolve({ id }),
  };
}

describe("POST /api/transactions/[id]/contest", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("rejects an unauthenticated request without ever calling the RPC", async () => {
    ...
    const { POST } = await import("@/app/api/transactions/[id]/contest/route");
  ...
  it("surfaces a 400 when contester_livraison_fan itself rejects the call", async () => {
    vi.mocked(createSupabaseServerClient).mockResolvedValue(
      buildSupabase(
        { id: "u1" },
  ...
  it("calls contester_livraison_fan with the transaction id on success, never attempting a refund", async () => {
    const rpcSpy = vi.fn();
    vi.mocked(createSupabaseServerClient).mockResolvedValue(
    ...
    expect(rpcSpy).toHaveBeenCalledWith("contester_livraison_fan", {
